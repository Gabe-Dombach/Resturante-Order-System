$(document).ready(function () {
  $(".loginBut").click(function () {
    ...
    if (
      ...
      if ($(".passwordLogin").val() == "password") {//Check to see if credentials are the managers
        window.location.replace("../manager-page/manager.html");
      } else window.alert("incorect password");
    } else {
      let currentUser = JSON.parse(localStorage.getItem($(".emailLogin").val()));//pull up current user and set current user in localStorage
      localStorage.setItem('userEmail',JSON.stringify($('.emailLogin').val()));
      if ($(".passwordLogin").val() == currentUser.password) {//check if password is correct
  ...
  $("*").keyup(function (event) {//Event listner to trigger login function when enter is pressed
    if (event.keyCode === 13) {
      $(".loginBut").click();
    }
  });
